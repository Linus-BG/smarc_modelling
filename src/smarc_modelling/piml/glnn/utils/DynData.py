# https://github.com/katfriedl/reduced_lagrangians/blob/main/utils/DynData.py
...
class DynDataset(Dataset):
    ...
    def __init__(self, n_dof, file_path, n_rec=[], phase='train', sampling_mode='random', n_samples=300, n_delta_t=1, device=None):
        ...
        # Load data
        eta, dq, _, _, _, _, _, tau, t = prepare_data(file_path)
        # Note dq already comes as accelerations in euler format

        # Convert quat to euler
        pos = eta[: ,:3]
        quat = eta[:, 3:]
        euler = R.from_quat(quat).as_euler("xyz", degrees=False)
        q = np.hstack([pos, euler])
