...
# Based on https://github.com/katfriedl/reduced_lagrangians/blob/main/models/geom_LNN.py
# Modified to train D instead of M
...
from smarc_modelling.piml.glnn.utils.spd_manifold import SPD as SPDManifold
import smarc_modelling.piml.glnn.utils.autodiff as numdiff
...
class LNN(nn.Module):
    ...
    def __init__(self, n_dof, **kwargs):
        ...
        # Model for SAM that calculates the different needed matrices
        self.sam = SAM(dt=0.01)
        ...
        self.out_D = torch.zeros(0, self.n_dof, self.n_dof)
        ...
        self.layers_D.append(EuclideanLayer(self.n_dof, self.n_width_T_eucl, activation=self.activation, device=self.device))
        init_layer(self.layers_D[-1])
        for _ in range(1, self.n_depth_T_eucl):
            self.layers_D.append(
                EuclideanLayer(self.n_width_T_eucl, self.n_width_T_eucl, activation=self.activation, device=self.device))
            init_layer(self.layers_D[-1])
        self.layers_D.append(EuclideanLayer(self.n_width_T_eucl, self.m, activation="Linear", device=self.device))
        init_layer(self.layers_D[-1], is_out=True)

        # expmap-layer
        self.layers_D.append(SPDExpLayer(self.n_dof, parametrized_expmap=self.do_exp_wrt_p, device=self.device))
    ...
    def fw_prediction(self, q, dq, tau, save_outputs=True):
        ...
        # Added D into the dynamics model and generates other matrices from SAM model
        ...
        if save_outputs:
            if self.out_M.shape[0] == 0:
                ...
                self.out_D = torch.cat((self.out_D, D.unsqueeze(1)), dim=1)
